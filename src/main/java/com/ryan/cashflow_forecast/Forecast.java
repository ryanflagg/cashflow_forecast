    ...
    public void createForecast(){
        ...
        for (int mth = 0; mth < COUNT_MONTHS; mth++){
            ...
            for (int i = 0; i < transactions.size(); i++){
                ...
                showMessageDialog(null,schedule);
                
                
                try {
                    //showMessageDialog(null,mth + " " + schedule.get(mth));
                // split income and expenditure
                    if ("Income".equals(type) && schedule.get(mth) == 1){
                        float amt = t.getAmount();
                        monthIncomeTotal = monthIncomeTotal + amt;               
                    }
            
                    if ("Expenditure".equals(type) && schedule.get(mth) == 1){
                        float amt = t.getAmount();
            ...
            try{
                ...
                //showMessageDialog(null, "month inc = " + monthIncomeTotal);
                expenditure.add(monthExpTotal);
                //showMessageDialog(null, "month exp = " + monthExpTotal);
                surplus.add(mthSurplus);
                //showMessageDialog(null, "month surplus = " + mthSurplus);
            }
